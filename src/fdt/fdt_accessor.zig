...
pub const FdtNodeProp = struct {
    ...
    pub fn fromPtr(ptr: [*]const u8, accessor: *Accessor) FdtNodeProp {
    ...
    pub fn printValue(self: *const FdtNodeProp) void {
        ...
        if(
            ('a' <= self.data[i] and self.data[i] <= 'z') or
...
pub const Accessor = struct {
    ...
    pub fn init(fdt_base: [*]const u8, fdt_header: *const types.FdtHeader) Accessor {
        return .{
            .structs = .{
                .base = fdt_base + fdt_header.off_dt_struct,
                .end = fdt_base + fdt_header.off_dt_struct + fdt_header.size_dt_struct
            },
            .strings = .{
                .base = fdt_base + fdt_header.off_dt_strings,
                .end = fdt_base + fdt_header.off_dt_strings + fdt_header.size_dt_strings
            },
        };
    }

    pub fn asToken(self: *Accessor, cur_ptr: [*]const u8) ?Token {
        if(pointer.gte([*]const u8, cur_ptr, self.structs.end)) return null;
    ...
    pub fn asByte(self: *Accessor, cur_ptr: [*]const u8) ?u8 {
        if(pointer.gte([*]const u8, cur_ptr, self.structs.end)) return null;
        return cur_ptr[0];
    }

    pub fn stringAddrFromOffset(self: *Accessor, offset: usize) ?[*]const u8 {
        const ptr = self.strings.base + offset;
        if(pointer.gte([*]const u8, ptr, self.strings.end)) return null;
        return ptr;
    }

    pub fn printStringFromOffset(self: *Accessor, offset: usize) void {
    ...
    pub fn getPropByName(self: *Accessor, node: FdtNode, name: []const u8) ?FdtNodeProp {
        var cur_ptr: [*]const u8 = pointer.align4(node + 4);

        while(self.asToken(cur_ptr)) |tok| : (cur_ptr = pointer.align4(cur_ptr)) {
            switch(tok) {
                .nop => {
                    cur_ptr += 4;
    ...
    pub fn findNodeWithProp(self: *Accessor, node: FdtNode, name: []const u8) ?FdtNode {
        var cur_ptr: [*]const u8 = pointer.align4(node);
        
        while(self.asToken(cur_ptr)) |tok| :
            (cur_ptr = pointer.align4(cur_ptr + 4)) {
            switch(tok) {
                .begin_node => {
    ...
    pub const FindParentResult = union(enum) {
        Found: ?FdtNode,
        EndNode: [*]const u8,
        NotFound: void
    };

    fn findParentRecursive(self: *Accessor, cur_node: FdtNode, node: FdtNode) FindParentResult {
    ...
    pub fn findParent(self: *Accessor, node: FdtNode) ?FdtNode {
    ...
    pub fn findNode(self: *Accessor, node: FdtNode, starts_with: []const u8) ?FdtNode {
        var cur_ptr: [*]const u8 = pointer.align4(node);
    ...
    pub fn printNode(self: *Accessor, node: FdtNode) void {
        var cur_ptr = node;
        var indent: usize = 0;
        var depth: usize = 0;
