...
pub fn identityMapKernel(
    ...
    kernel_global.KERNEL_VIRT_OFFSET = kernel_global.KERNEL_VIRT_BASE - kernel_bounds.kstart;
...
fn patchVectorTable(kernel_bounds: *const kernel_global.KernelBounds) void {
    const vector_table_addresses: []usize = @as([*]usize, @ptrFromInt(kernel_bounds.kstart + 0x24))[0..7];
    for(0..vector_table_addresses.len) |i| {
        vector_table_addresses[i] = kernel_global.physToVirt(vector_table_addresses[i]);
    }
}
...
pub fn mapToHigherAddress(
    ...
    try kernel_virt_mem.kernelMapSection(kernel_global.VECTOR_TABLE_BASE, kernel_bounds.kstart);
    ...
    page_alloc.global_page_alloc = @ptrFromInt(kernel_global.physToVirt(@intFromPtr(page_alloc.global_page_alloc)));

    uart.print("fdt map : {x} -> {x}\n", .{kernel_global.physToVirt(@intFromPtr(fdt_base)), @intFromPtr(fdt_base)});
    uart.print("vector table : {x} -> {x}\n", .{kernel_global.VECTOR_TABLE_BASE, kernel_bounds.kstart});

    uart.print("---------------mapping higher addr end---------------------\n\n", void);
}

pub fn transitionToHigherHalf(
    _: *const kernel_global.KernelBounds,
    kernel_virt_mem: *virt_mem_handler.VirtMemHandler,
    higher_half_main: *const fn ([*]const u8) void,
    fdt_base: [*]const u8
) noreturn {
    arm.enableMMU(@intFromPtr(kernel_virt_mem.l1));
    kernel_virt_mem.l1 = @as(*page_table.L1PageTable, @ptrFromInt(kernel_global.physToVirt(arm.ttbr.read(1))));
