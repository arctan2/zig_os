...
pub fn DebugAllocator(comptime config: Config) type {
    return struct {
        ...
        fn collectStackTrace(_: usize, addresses: *[stack_n]usize) void {
            if (stack_n == 0) return;
            @memset(addresses, 0);
        ...
        fn reportDoubleFree(_: usize, alloc_stack_trace: StackTrace, free_stack_trace: StackTrace) void {
            var addresses: [stack_n]usize = @splat(0);
            const second_free_stack_trace: StackTrace = .{
                .instruction_addresses = &addresses,
                .index = 0,
            };
        ...
        fn resizeLarge(
            ...
            if (config.safety and old_mem.len != entry.value_ptr.bytes.len) {
                ...
                const free_stack_trace: StackTrace = .{
                    .instruction_addresses = &addresses,
                    .index = 0,
                };
